...
public class BidListServiceTest {
	...
	public void whenFindAll_thenReturnsListOfAllBidLists() {
		...
		given(bidListMapping.mapAListOfBidList(listOfBidList)).willReturn(listOfBidListDTO);
		// WHEN
	...
	public void givenABidListDTO_whenSave_thenReturnsSavedObject() {
		// GIVEN
		given(bidListMapping.mapDTOToEntity(listOfBidListDTO.get(0))).willReturn(listOfBidList.get(0));
		given(bidListRepository.save(listOfBidList.get(0))).willReturn(listOfBidList.get(0));
		given(bidListMapping.mapEntityToDTO(any(BidList.class))).willReturn(listOfBidListDTO.get(0));		
		// WHEN
		BidListDTO result = bidListService.save(listOfBidListDTO.get(0));
		// THEN
		assertThat(result.toString()).isEqualTo(listOfBidListDTO.get(0).toString());
	}

	@Test
	public void whenGetById_thenReturnsExpectedBidList() {
		// GIVEN
		given(bidListRepository.findByBidListId(2)).willReturn(listOfBidList.get(1));
		given(bidListMapping.mapEntityToDTO(any(BidList.class))).willReturn(listOfBidListDTO.get(1));		
		// WHEN
		BidListDTO result = bidListService.getById(2);
		// THEN
		assertThat(result.toString()).isEqualTo(listOfBidListDTO.get(1).toString());
	}
	...
	public void givenABidListDTO_whenDelete_thenReturnsDeletedBidList() {
		...
		given(bidListRepository.findByBidListId(3)).willReturn(listOfBidList.get(2));
		given(bidListMapping.mapEntityToDTO(any(BidList.class))).willReturn(listOfBidListDTO.get(2));		
		// WHEN
		BidListDTO result = bidListService.delete(3);
		// THEN
		assertThat(result.toString()).isEqualTo(listOfBidListDTO.get(2).toString());	
	}
